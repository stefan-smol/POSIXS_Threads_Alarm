...
{
    int exists = 0;                            // Flag to check if a thread for the group already exists.
    pthread_mutex_lock(&display_thread_mutex); // Lock the mutex to safely access the display thread list.

    display_thread_t *current_thread = display_thread_list; // Start from the head of the display thread list.
    while (current_thread != NULL)
    { // Iterate over the display thread list.
        if (current_thread->time_group_number == group_number)
        {               // Check if a thread for the group exists.
            exists = 1; // Set the flag if a thread is found.
            break;      // Exit the loop as a thread is found.
        }
        current_thread = current_thread->next; // Move to the next thread in the list.
    }

    if (!exists)
    {                                                                                     // If no thread exists for the group, create a new one.
        pthread_t new_thread;                                                             // Declare a variable for the new thread.
        int *time_group = malloc(sizeof(int));                                            // Dynamically allocate memory to store the group number.
        *time_group = group_number;                                                       // Set the group number in the allocated memory.
        int status = pthread_create(&new_thread, NULL, display_alarm_thread, time_group); // Create a new thread.
        if (status != 0)
        {
            err_abort(status, "Create display alarm thread"); // Error handling.
        }

        // Allocate memory for the new display thread structure and add it to the list.
        ...
        new_display_thread->thread_id = new_thread;           // Set the thread ID.
        new_display_thread->time_group_number = group_number; // Set the group number.
        new_display_thread->next = display_thread_list;       // Insert the new thread at the beginning of the list.
        display_thread_list = new_display_thread;             // Update the head of the list.

        printf("Created New Display Alarm Thread %lu for Alarm_Time_Group_Number %d to Display Alarm(%d) at %ld: %d [Message]\n",
               (unsigned long)new_thread, group_number, alarm_id, now); // Print confirmation.
    }

    pthread_mutex_unlock(&display_thread_mutex); // Unlock the mutex.
}
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
                   ...
                   alarm->alarm_category, alarm->message) == 4)
        {
            ...
            // Unlock the mutex after modifying the alarm list.
            // This is important to allow other threads to access the list.
            status = pthread_mutex_unlock(&alarm_mutex);
            if (status != 0)
            {
                err_abort(status, "Unlock mutex"); // Abort if mutex unlock fails.
            }
        }
